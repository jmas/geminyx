...
export function DialogListScreen() {
  ...
  const { result } = useList<Dialog>({
    resource: RESOURCES.dialogs,
    pagination: { mode: "off" },
  });
  const dialogs = useMemo(() => result.data ?? [], [result.data]);
        ...
        selecting ? (
          <Pressable
            onPress={() => {
              ...
              const first = dialogs.find((d) => d.id === selectedIds[0]);
              confirmDeleteIds(
                selectedIds,
...
function DialogRow({
  ...
  const { capsule } = dialog;
  const subtitle = formatLastMessageDate(dialog.lastMessageAt);
